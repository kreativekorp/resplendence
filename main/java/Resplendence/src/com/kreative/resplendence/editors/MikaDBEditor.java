...
public class MikaDBEditor implements ResplendenceEditor {
	public Image largeIcon() {
		return ResplRsrcs.getPNG("DATABASE", "MikaDB");
	}

	public String name() {
		return "MikaDB Database Editor";
	}

	public ResplendenceEditorWindow openEditor(ResplendenceObject ro) {
		return new WMikaDBTablePicker(ro);
	}

	public int recognizes(ResplendenceObject ro) {
		if (ro.isContainerType() && ro.getType().equals(ResplendenceObject.TYPE_DATABASE_DATABASE) && (ro.getTitleForWindowMenu().contains("MikaDB") || ro.getTitleForWindowMenu().contains("mikadb"))) {
			return PREFERRED_EDITOR+1;
		} else {
	...
	public Image smallIcon() {
		return ResplUtils.shrink(ResplRsrcs.getPNG("DATABASE", "MikaDB"));
	}
	
	public static class WMikaDBTablePicker extends ResplendenceEditorWindow {
		...
		public WMikaDBTablePicker(ResplendenceObject ro) {
			super(ro, true);
			...
			try {
				dp = new DFFResourceDatabase(conn, "dff");
			} catch (Exception e) {
				dp = null;
			}
			...
			thumbList.addMouseListener(new MouseAdapter() {
				public void mouseClicked(MouseEvent ev) {
					if (ev.getClickCount() == 2) {
						...
						for (int row : rows) {
							WMikaDBTablePicker.this.resplOpen((ResplendenceObject)thumbModel.get(row));
						}
					}
				}
			});
			...
			listTable.addMouseListener(new MouseAdapter() {
				public void mouseClicked(MouseEvent ev) {
					if (ev.getClickCount() == 2) {
						...
						for (int row : rows) {
							WMikaDBTablePicker.this.resplOpen((ResplendenceObject)listModel.getValueAt(row, 0));
						}
					}
				}
			});
		...
		public Image getTableIcon(String table) {
			if (dp != null) {
				try {
					PreparedStatement ps = conn.prepareStatement("SELECT tableicon FROM mikadb WHERE tablename=?");
					ps.setString(1, table);
					ResultSet rs = ps.executeQuery();
					if (rs.next() && rs.getInt(1) != 0) {
						byte[] stuff = dp.getData(KSFLConstants.ImagePNG, rs.getInt(1));
						if (stuff != null && stuff.length > 0) return this.getToolkit().createImage(stuff);
					}
				} catch (SQLException se) {
					byte[] stuff = dp.getData(KSFLConstants.ImagePNG, table);
					if (stuff != null && stuff.length > 0) return this.getToolkit().createImage(stuff);
				}
			}
			return ResplRsrcs.getPNG("DBTABLE", table);
		}
		...
		public void update() {
			...
			for (ResplendenceObject ro : things) {
				...
				thumbIcons.put(ro, new ImageIcon(getTableIcon(ro.getTitleForIcons())));
			}
