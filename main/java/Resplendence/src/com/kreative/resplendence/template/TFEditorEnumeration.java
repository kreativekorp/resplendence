package com.kreative.resplendence.template;

import java.awt.*;
import java.util.*;
import java.util.List;
import javax.swing.*;

public class TFEditorEnumeration extends JComboBox implements TFEditor {
	private static final long serialVersionUID = 1;
	
	private int w;
	private boolean l;
	private Map<Long,String> opt = new HashMap<Long,String>();
	
	public TFEditorEnumeration(int bitWidth, boolean littleEndian, List<TemplateField> options) {
		this.setFont(mono);
		this.setEditable(false);
		this.setRenderer(new MyCellRenderer());
		w = bitWidth;
		l = littleEndian;
		long lastValue = -1;
		Iterator<TemplateField> i = options.iterator();
		while (i.hasNext()) {
			String[] ss = i.next().getName().split("=",2);
			if (ss.length < 2) {
				lastValue++;
				lastValue <<= (64-bitWidth);
				lastValue >>= (64-bitWidth);
				opt.put(lastValue, ss[0].trim());
				this.addItem(lastValue);
			} else {
				try {
					lastValue = Long.parseLong(ss[0].trim());
				} catch (NumberFormatException e) {
					lastValue++;
				}
				lastValue <<= (64-bitWidth);
				lastValue >>= (64-bitWidth);
				opt.put(lastValue, ss[1].trim());
				this.addItem(lastValue);
			}
		}
	}
	
	private long bytesToValue(byte[] b) {
		long v = 0;
		if (l) {
			for (int i=b.length-1; i>=0; i--) {
				v <<= 8;
				v |= (b[i] & 0xFF);
			}
		} else {
			for (int i=0; i<b.length; i++) {
				v <<= 8;
				v |= (b[i] & 0xFF);
			}
		}
		v <<= (8*(8-b.length));
		v >>= (8*(8-b.length));
		return v;
	}
	
	private long booleansToValue(boolean[] b) {
		long v = 0;
		if (l) {
			for (int i=b.length-1; i>=0; i--) {
				v <<= 1;
				if (b[i]) v |= 1;
			}
		} else {
			for (int i=0; i<b.length; i++) {
				v <<= 1;
				if (b[i]) v |= 1;
			}
		}
		v <<= (64-b.length);
		v >>= (64-b.length);
		return v;
	}
	
	private byte[] valueToBytes(long v, int nb) {
		byte[] b = new byte[nb];
		if (l) {
			for (int i=0; i<b.length; i++) {
				b[i] = (byte)(v & 0xFF);
				v >>= 8;
			}
		} else {
			for (int i=b.length-1; i>=0; i--) {
				b[i] = (byte)(v & 0xFF);
				v >>= 8;
			}
		}
		return b;
	}
	
	private boolean[] valueToBooleans(long v, int nb) {
		boolean[] b = new boolean[nb];
		if (l) {
			for (int i=0; i<b.length; i++) {
				b[i] = ((v & 1) > 0);
				v >>= 1;
			}
		} else {
			for (int i=b.length-1; i>=0; i--) {
				b[i] = ((v & 1) > 0);
				v >>= 1;
			}
		}
		return b;
	}
	
	private long parseLong(String s) {
		try {
			return Long.parseLong(s);
		} catch (NumberFormatException e) {
			return 0;
		}
	}
	
	public Position readValue(byte[] data, Position pos) {
		if ((w & 7) == 0) {
			byte[] stuff = new byte[w>>>3];
			pos = pos.getBytes(data, stuff);
			long v = bytesToValue(stuff);
			this.setSelectedItem(v);
		} else {
			boolean[] stuff = new boolean[w];
			pos = pos.getBits(data, stuff);
			long v = booleansToValue(stuff);
			this.setSelectedItem(v);
		}
		return pos;
	}
	
	public Position writeValue(byte[] data, Position pos) {
		if ((w & 7) == 0) {
			Object o = this.getSelectedItem();
			long v = (o instanceof Number)?(((Number)o).longValue()):parseLong(o.toString());
			byte[] stuff = valueToBytes(v, w>>>3);
			pos = pos.setBytes(data, stuff);
		} else {
			Object o = this.getSelectedItem();
			long v = (o instanceof Number)?(((Number)o).longValue()):parseLong(o.toString());
			boolean[] stuff = valueToBooleans(v, w);
			pos = pos.setBits(data, stuff);
		}
		return pos;
	}

	public Position writeValue(Position pos) {
		return pos.skipBits(w);
	}
	
	private class MyCellRenderer extends DefaultListCellRenderer {
		private static final long serialVersionUID = 1;
		@Override
		public Component getListCellRendererComponent(JList list, Object value, int index, boolean sel, boolean focus) {
			if ((opt != null) && (value instanceof Number) && (opt.containsKey(((Number)value).longValue()))) {
				return super.getListCellRendererComponent(list, opt.get(((Number)value).longValue()), index, sel, focus);
			} else {
				return super.getListCellRendererComponent(list, value, index, sel, focus);
			}
		}
	}
}
